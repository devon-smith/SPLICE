...
PAIR_FEATURE_DIM_HADAMARD = PAIR_FEATURE_DIM + EMBEDDING_DIM  # + e_L * e_R = 3073
...
def build_pair_features_batch(
    e_left: np.ndarray, e_right: np.ndarray, include_hadamard: bool = False
) -> np.ndarray:
    """Vectorised feature build.

    Default: ``(n,768),(n,768) -> (n,2305)`` float32 with layout
    ``[eL | eR | |eL-eR| | cos]``.

    When ``include_hadamard=True`` the Hadamard (elementwise) product ``eL * eR``
    is appended *before* the cosine column, giving ``(n,3073)`` with layout
    ``[eL | eR | |eL-eR| | eL*eR | cos]``. The product term is a standard
    siamese-verification feature; concretely it gives the head an axis-wise
    "agreement" signal that the absolute-difference + cosine pair does not.
    """
    ...
    parts: list[np.ndarray] = [e_l, e_r, abs_diff]
    if include_hadamard:
...
def build_pair_feature(
    ...
    """Single-pair feature: ``(768,),(768,) -> (2305,)`` (or 3073 with Hadamard)."""
